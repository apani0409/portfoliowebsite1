...
// hook\import { useTheme } from '../context/ThemeContext'
  ...
  return (
    ...
    >
      <div className='line relative h-16 overflow-hidden'>
        <span className='text absolute invisible'>Hello,</span>
      </div>
      <div className='line relative h-16 overflow-hidden'>
        <span
          style={{ color: theme.main_color }}
          className='text absolute invisible'
        >
...
function ProfileDescription({ theme }: { theme: any }) {
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      q('span'),
      { autoAlpha: 0 },
      { autoAlpha: 1, delay: 1.2, duration: 0.5 }
    )
  }, [])
...
const ProfileLink = React.forwardRef(({ theme }: { theme: any }, ref) => {
  const container = useRef(null)
  const q = gsap.utils.selector(container)
  ...
  return (
    <div ref={container} style={{ color: theme.main_color }} className='flex gap-10 text-sm 2xl:text-sm'>
      <a
        ...
        download='Alessandro Pani Resume'
        href={resume}
        target='_blank'
      >
      ...
      <ProfileLinkItem
        text='LinkedIn'
        link='https://www.linkedin.com/in/alessandro-pani-931945245/'
        icon={<TriangleRightFill strokeWidth={2} size={20} className='ml-1' />}
      />
      <ProfileLinkItem
        text='Github'
        link='https://github.com/apani0409'
        icon={<TriangleRightFill strokeWidth={2} size={20} className='ml-1' />}
      />
